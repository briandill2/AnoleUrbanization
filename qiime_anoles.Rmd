...
library(ANCOMBC)
library(microbiome)
#library(nplotr) didnt work dont know why or what this is even for
library(rmeta)
library(readr)
library(vegan)
```
...
## breakdown the full ps object then prune bad samples
...
## Make all of the required ps objects. for each first combine and then rarefy
...
#psUM.DCA.bray = ordinate(psUM, "DCA", distance= psUM.distance.bray)
#psUM.CCA.bray = ordinate(psUM, "CCA", distance= psUM.distance.bray)
#psUM.RDA.bray = ordinate(psUM, "RDA", distance= psUM.distance.bray)
...
#psUM.MDS.bray = ordinate(psUM, "MDS", distance= psUM.distance.bray)

psUM.NMDS.bray
psUM.NMDS.dice


#psUM.DCA.bray.plot <- plot_ordination(psUM, psUM.DCA.bray, color="description", title = "DCA")
#psUM.CCA.bray.plot <- plot_ordination(psUM, psUM.CCA.bray,type = "life_stage", color="description", title = "CCA")
#psUM.RDA.bray.plot <- plot_ordination(psUM, psUM.RDA.bray, color="description", title = "RDA")
plot_ordination(psUM.rare, psUM.NMDS.bray, color="description", title = "NMDS")
#psUM.MDS.bray.plot <- plot_ordination(psUM, psUM.MDS.bray, color="description", title = "MDS")


# all the ordination methods for Bray

dist = psM.rare.distance.dice
ord_meths = c("DCA", "CCA", "RDA", "NMDS", "MDS", "PCoA")

plist = llply(as.list(ord_meths), function(i, physeq, dist){
        ordi = ordinate(physeq, method=i, distance=dist)
        plot_ordination(physeq, ordi, color="description")
}, psM.rare, dist)
...
ggplot(pdataframe, aes(Axis_1, Axis_2, color=description)) + geom_point() + facet_wrap(~method, scales="free") + scale_color_manual(values = c("#08306B", "#2171B5", "#C6DBEF", "#6BAED6")) + theme_dark() + ggtitle(label= "psM dice")
...
ggplot(pdataframe, aes(Axis_1, Axis_2, color=description)) + geom_point() + facet_wrap(~method, scales="free") + scale_color_manual(values = c("#08306B", "#2171B5", "#C6DBEF", "#6BAED6", "#e3c342")) + theme_dark() + ggtitle(label= "psUM dice")
...
psUMC.duo.PCoA.bray.plot
# psUM.PCoA.jacc.plot
# psUM.PCoA.bray.plot

# all the ordination methods for Bray and Jacc. Just rerun with dist= what you want

dist = psUMC.duo.distance.dice
ord_meths = c("DCA", "CCA", "RDA", "NMDS", "MDS", "PCoA")
...
ggplot(pdataframe, aes(Axis_1, Axis_2, color=description)) + geom_point() + facet_wrap(~method, scales="free") + scale_color_manual(values = c("#08306B", "#2171B5", "#C6DBEF", "#6BAED6", "#03fc4d", "#228c42", "#205731", "#e3c342")) + theme_dark() + ggtitle(label= "psUMC.duo dice")
...
ggplot(pdataframe, aes(Axis_1, Axis_2, color=description)) + geom_point() + facet_wrap(~method, scales="free") + scale_color_manual(values = c("#08306B", "#2171B5", "#C6DBEF", "#6BAED6", "#03fc4d", "#228c42", "#e3c342")) + theme_dark() + ggtitle(label= "psUMC bray")


ggsave("figures/psUMC_Bray_NMDS.pdf", device = "pdf", width = 9, height = 9)
```
...
## ANCOM-BC lets do some differential abundance testing
...
testps <- subset_samples(psUMC.rare, description == "Mayaguez Urban" | description == "Maricao Rocks")
sample_data(testps)

phylum_data = aggregate_taxa(testps, "Genus")

otu_table(phylum_data)


out = ancombc(phyloseq = testps, formula = "description", 
              p_adj_method = "holm", zero_cut = 0.90, lib_cut = 1000, 
              group = "description", struc_zero = TRUE, neg_lb = TRUE, tol = 1e-5, 
...
sample_data(phylum_data)$description = recode(sample_data(phylum_data)$description, "AMayaguez Urban" = "Mayaguez Urban", USA = "AUSA")


table(meta(phylum_data)[, "description"])
...
## Going to try out ANCOMII instead of ANCOMBC because of the plotting funcitonality
``` {r, echo = T, eval = T}
library(nlme)
library(tidyverse)
library(ggplot2)
...
write_csv(res$out, "test/ANCOM_test.csv")
...
## Various Heatmaps Mostly with the above ancom results.
``` {r, echo = T, eval = T}

library(scales)

#MU_ABnB_ASV_heat
MU_ABnB_ASV_heat <- subset_samples(psM.rare, description == "Mayaguez Urban" | description =="AirBnB")
MU_ABnB_ASV_heat <- prune_taxa("ASV16414", MU_ABnB_ASV_heat)
tax_table(MU_ABnB_ASV_heat)
fig_MU_ABnB_ASV_heat <- plot_heatmap(MU_ABnB_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family", title = "MU_ABnB")



#MU_MF_ASV_heat
MU_MF_ASV_heat <- subset_samples(psM.rare, description == "Mayaguez Urban" | description =="Mayaguez Forest")
MU_MF_ASV_heat <- prune_taxa(c("ASV6593", "ASV7025", "ASV11740"), MU_MF_ASV_heat)
tax_table(MU_MF_ASV_heat)
fig_MU_MF_ASV_heat <- plot_heatmap(MU_MF_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family", title = "MU_MF")


#MU_MR_ASV_heat
MU_MR_ASV_heat <- subset_samples(psM.rare, description == "Mayaguez Urban" | description =="Maricao Rocks")
MU_MR_ASV_heat <- prune_taxa(c("ASV4663", "ASV6593", "ASV7025", "ASV7280", "ASV8582", "ASV13425", "ASV16638", "ASV22190", "ASV26624", "ASV26736"), MU_MR_ASV_heat)
tax_table(MU_MR_ASV_heat)
fig_MU_MR_ASV_heat <- plot_heatmap(MU_MR_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family", title = "MU_MR")


#MU_MR_ASV_heat + usa humans
H_MU_MR_ASV_heat <- subset_samples(psUM.rare, description == "Mayaguez Urban" | description =="Maricao Rocks" | description =="USA")
H_MU_MR_ASV_heat <- prune_taxa(c("ASV4663", "ASV6593", "ASV7025", "ASV7280", "ASV8582", "ASV13425", "ASV16638", "ASV22190", "ASV26624", "ASV26736"), H_MU_MR_ASV_heat)
tax_table(H_MU_MR_ASV_heat)
fig_H_MU_MR_ASV_heat <- plot_heatmap(H_MU_MR_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family", title = "MU_MR_USA")
plot_heatmap(H_MU_MR_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", title = "MU_MR_USA")


#MU_MR_ASV_heat + venez humans
V_MU_MR_ASV_heat <- subset_samples(psVM.rare, description == "Mayaguez Urban" | description =="Maricao Rocks" | description =="Venezuela")
V_MU_MR_ASV_heat <- prune_taxa(c("ASV4663", "ASV6593", "ASV7025", "ASV7280", "ASV8582", "ASV13425", "ASV16638", "ASV22190", "ASV26624", "ASV26736"), V_MU_MR_ASV_heat)
tax_table(V_MU_MR_ASV_heat)
fig_V_MU_MR_ASV_heat <- plot_heatmap(V_MU_MR_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family", title = "MU_MR_V")
plot_heatmap(V_MU_MR_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", title = "MU_MR_V")

#MU_MR_ASV_heat + Malawi humans
M_MU_MR_ASV_heat <- subset_samples(psMAM.rare, description == "Mayaguez Urban" | description =="Maricao Rocks" | description =="Malawi")
M_MU_MR_ASV_heat <- prune_taxa(c("ASV4663", "ASV6593", "ASV7025", "ASV7280", "ASV8582", "ASV13425", "ASV16638", "ASV22190", "ASV26624", "ASV26736"), M_MU_MR_ASV_heat)
tax_table(M_MU_MR_ASV_heat)
fig_M_MU_MR_ASV_heat <- plot_heatmap(M_MU_MR_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family", title = "MU_MR_M")
plot_heatmap(M_MU_MR_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", title = "MU_MR_M")

#coyotes_ASV_heat
coyotes_ASV_heat <- subset_samples(psC.rare, description != "Urban coyote duodenal samples")
coyotes_ASV_heat <- prune_taxa(c("ASV19872", "ASV28851"), coyotes_ASV_heat)
tax_table(coyotes_ASV_heat)
fig_coyotes_ASV_heat <- plot_heatmap(coyotes_ASV_heat, sample.label = "description", method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family", title = "coyotes")




# Top 30 ASV heatmap not related to ANCOM

top30asv <- prune_taxa(names(sort(taxa_sums(psAll.rare),TRUE)[1:30]), psAll.rare)
top30asv_heat <- plot_heatmap(top30asv, method = FALSE, high = "#d95f0e", low = "#fff7bc", na.value = "#ffffff", taxa.label = "Family") + facet_wrap(~description, scales="free_x", nrow = 1)
...
fig_MU_ABnB_ASV_heat
fig_MU_MF_ASV_heat
fig_MU_MR_ASV_heat
fig_H_MU_MR_ASV_heat
fig_V_MU_MR_ASV_heat
fig_M_MU_MR_ASV_heat
fig_coyotes_ASV_heat + facet_wrap(~description, scales="free_x")
top30asv_heat
...
ggsave("figures/top30asv_heat.pdf", device = "pdf", width = 18, height = 8)
...
M.vegdist
#will need vegan meta eventually
